...
const DOCS3_DIR = join(process.cwd(), "public/images/docs3");
const OUTPUT_PATH = join(process.cwd(), "scripts/docs3-cloudinary-urls.json");

async function main() {
  const files = readdirSync(DOCS3_DIR).sort();
  const urls: Record<string, string> = {};

  for (const filename of files) {
    const filePath = join(DOCS3_DIR, filename);
    const publicId = basename(filename, extname(filename));

    console.log(`Uploading ${filename}...`);
    const result = await cloudinary.uploader.upload(filePath, {
      folder: "skyandsoul/docs3",
      public_id: publicId,
      overwrite: true,
      resource_type: "auto",
    });
